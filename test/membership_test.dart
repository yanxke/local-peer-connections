...
void main() {
  ...
  test(
    'membership snapshot validates canonical hash and record ordering',
    () async {
      ...
      encoded[30] ^= 1;
      expect(
        () => MembershipSnapshot.decode(encoded),
  ...
  test(
    'UT-275 excluded local member requires reconciliation, not disconnect',
    () async {
      final snapshot = MembershipSnapshot(
        groupId: GroupId(List.filled(16, 9)),
        coordinatorTerm: 4,
      ...
      expect(
        membershipSnapshotLocalDisposition(snapshot, _peer(3)),
        MembershipSnapshotLocalDisposition.excluded,
      );
      expect(
        membershipSnapshotLocalDisposition(snapshot, _peer(2)),
  ...
  test(
    'UT-101/102/103/COORD-052/053 new SessionId starts a fresh baseline',
    () {
      final ordering = MembershipSnapshotOrderTable();
      expect(
        ordering.observe(
  ...
  test(
    'COORD-047 newer same-term snapshot survives old post-RESUME replay',
    () async {
  ...
  test(
    'COORD-049/050 post-RESUME duplicate B ACKs without reapplying',
    () async {
      final receiver = MembershipSnapshotReceiver();
      final sessionId = List.filled(16, 3);
      final a = MembershipSnapshot(
  ...
  test(
    'COORD-048/051 resumed newer snapshot prevents stale rollback',
    () async {
      final receiver = MembershipSnapshotReceiver();
      final sessionId = List.filled(16, 3);
      final a = MembershipSnapshot(
