...
void main() {
  ...
  test(
    'UT-158/191 runtime preserves pre-auth discovery local-name metadata',
    () async {
  ...
  test(
    'discovery emits found, updated, and lost endpoint lifecycle events',
    () async {
  ...
  test(
    'currentEndpoints returns an immutable point-in-time snapshot',
    () async {
      final discovery = DiscoverySession();
  ...
  test(
    'UT-170 RuntimeConfig defaults to SAS and validates trust credentials',
    () {
      ...
      RuntimeConfig(
        ...
        psk32: List.filled(32, 9),
      ).validate();
      expect(
        () => RuntimeConfig(gattFragmentInactivityTimeoutMs: 999).validate(),
  ...
  test(
    'UT-188 a large known-peer database creates no runtime probe state',
    () async {
  ...
  test(
    'UT-159 runtime multiplexes compatible HostSession advertising demand',
    () async {
  ...
  test(
    'UT-193 automatic known-peer probes put runtime metadata in HELLO',
    () async {
      ...
      TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger
          .setMockMethodCallHandler(methods, (call) async {
            if (call.method == 'connectGatt') {
              events.add(
                const PlatformGattConnected('metadata-endpoint', 'central'),
              );
  ...
  test('UT-194 HostSession inherits and overrides runtime metadata', () async {
    ...
    events.add(
      const PlatformGattConnected('overridden-endpoint', 'peripheral'),
    );
  ...
  test(
    'UT-202 presentation refreshes advertising without changing identity',
    () async {
  ...
  test(
    'UT-196/197/198 shared Host, Group, and Discovery demand is multiplexed',
    () async {
      ...
      final host = runtime.createHostSession(HostConfig());
      await host.startAdvertising();
      final group = runtime.joinOrCreateGroup(
        GroupConfig(
          applicationNamespace: [1],
          groupJoinToken: List.filled(16, 3),
        ),
      );
      await Future<void>.delayed(Duration.zero);
  ...
  test(
    'UT-164 runtime close cancels every nonterminal connection attempt',
    () async {
  ...
  test(
    'ConnectionAttempt cancellation is idempotent after the first cancel',
    () async {
  ...
  test(
    'ConnectionAttempt fails when the platform never completes connect',
    () async {
  ...
  test(
    'HostSession close is idempotent and emits one terminal event',
    () async {
      final runtime = await createRuntime(
        identityStore: InMemoryIdentityStore(),
        platformBleBackend: PlatformBleBackend(
          methods: const MethodChannel('host-close-test'),
        ),
      );
      final host = runtime.createHostSession(HostConfig());
  ...
  test(
    'automatic known-peer probes time out and release their endpoint',
    () async {
  ...
  test(
    'automatic probe protocol failure is reported and releases its slot',
    () async {
      const methods = MethodChannel(
        'runtime-known-probe-protocol-failure-test',
      );
      final events = StreamController<PlatformBleEvent>.broadcast();
      final runtimeEvents = <RuntimeEvent>[];
      ...
      events.add(
        PlatformGattFragment(
          'malformed-endpoint',
          GattFragment(1, [1], end: true).encode(),
        ),
      );
      await _waitUntil(
        () => runtimeEvents.whereType<KnownPeerProbeFailed>().isNotEmpty,
      ...
      expect(
        runtimeEvents.whereType<KnownPeerProbeFailed>().single.error.code,
  ...
  test(
    'known-peer probes enforce concurrency and preserve dropped candidates',
    () async {
  ...
  test(
    'UT-259 serializes automatic physical GATT connect procedures',
    () async {
      const methods = MethodChannel(
        'runtime-known-probe-physical-connect-test',
      );
      final events = StreamController<PlatformBleEvent>.broadcast();
      ...
      // Android vendor stacks can accept both method calls but cannot initiate
      // two LE link procedures concurrently.  The second endpoint remains
      // queued until the first native GATT-connected event arrives.
      events.add(const PlatformEndpointFound('physical-a', rssi: -40));
      events.add(const PlatformEndpointFound('physical-b', rssi: -41));
      await Future<void>.delayed(const Duration(milliseconds: 20));
      expect(connectCalls, ['physical-a']);

      events.add(const PlatformGattConnected('physical-a', 'central'));
      await _waitUntil(() => connectCalls.length == 2);
      expect(connectCalls, ['physical-a', 'physical-b']);
      ...
      TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger
          .setMockMethodCallHandler(methods, (call) async {
            if (call.method == 'connectGatt') {
              connects++;
              events.add(
                const PlatformGattConnected('same-endpoint', 'central'),
              );
            }
            if (call.method == 'submitGattFragment') return 'submitted';
            return null;
  ...
  test(
    'failed transport probe backs off while advertisements continue',
    () async {
  ...
  test(
    'UT-165 HostSession snapshots peers and broadcasts an empty target set',
    () async {
      final runtime = await createRuntime(
        localPeerId: PeerId(List.filled(16, 1)),
        platformBleBackend: PlatformBleBackend(
          methods: const MethodChannel('host-empty-test'),
        ),
      );
      final host = runtime.createHostSession(HostConfig());
  ...
  test(
    'UT-244 busy native GATT probe backs off while endpoint closes',
    () async {
  ...
  test(
    'UT-171 HostSession validates credentials for its trust-mode override',
    () async {
      final runtime = await createRuntime(
        localPeerId: PeerId(List.filled(16, 1)),
        platformBleBackend: PlatformBleBackend(
          methods: const MethodChannel('host-trust-test'),
        ),
      );
      expect(
  ...
  test(
    'UT-172 disabled GATT cannot start discovery, host, or connection',
    () async {
      final runtime = await createRuntime(
      ...
      await expectLater(runtime.startDiscovery(), throwsA(isA<LpcException>()));
      expect(
        () => runtime.createHostSession(HostConfig()),
        throwsA(isA<LpcException>()),
      );
      await runtime.close();
    },
