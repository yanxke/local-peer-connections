...
Future<HandshakeExchange> _exchange(
  ...
  final identity = await Ed25519().newKeyPairFromSeed(
    List.filled(32, identitySeed),
  );
  final ephemeral = await X25519().newKeyPairFromSeed(
    List.filled(32, ephemeralSeed),
  );
  ...
  return HandshakeExchange(
    ...
    tofuStore: trustMode == HandshakeTrustMode.tofu
        ? TofuIdentityStore()
        : null,
  );
}
...
void main() {
  test(
    'UT-274 silent HELLO/AUTH handshake times out and closes its backend',
    () async {
      final backend = _Backend('silent-handshake', forwardWrites: false);
      final exchange = await _exchange(7, 9);
      final logs = <String>[];
      final handshake = HandshakeConnection(
        backend: backend,
        exchange: exchange,
        localPeerId: exchange.localHello.peerId,
        candidateOnly: true,
        logger: logs.add,
      );
      // Observe both terminal futures: the candidate result is the expected
      // assertion, while normal readiness also fails on the same timeout.
      unawaited(
        handshake.ready.then<void>(
          (_) {},
      ...
      expect(logs, contains(contains('HELLO/AUTH handshake timed out')));
    },
  );
  ...
  test(
    ...
    () async {
      ...
      final cores = await Future.wait([
        a.ready,
        b.ready,
      ]).timeout(const Duration(seconds: 2));

      expect(
        cores.map((core) => core.state),
        everyElement(PeerConnectionState.ready),
      );
      expect(
        cores.map((core) => core.securityLevel),
  ...
  test(
    'UT-169 SAS handshake exposes both verification values before READY',
    () async {
      final backendA = _Backend('sas-a');
      final backendB = _Backend('sas-b');
      backendA.remote = backendB;
      backendB.remote = backendA;
      final exchangeA = await _exchange(
        11,
        13,
        trustMode: HandshakeTrustMode.sas,
      );
      final exchangeB = await _exchange(
        12,
        14,
        trustMode: HandshakeTrustMode.sas,
      );
  ...
  test(
    'UT-177 candidate handshake authenticates without normal READY',
    () async {
      ...
      expect(
        results.map((result) => result.secrets.sessionRootKey),
      ...
      expect(
        [
          ...backendA.writes,
          ...backendB.writes,
        ].map(LpcFrame.decode).where((frame) => frame.type == FrameType.ready),
        isEmpty,
      );
    },
  );
  ...
  test(
    'diagnostic logger reports handshake phases without frame payloads',
    () async {
  ...
  test(
    'a throwing handshake diagnostic logger cannot stop authentication',
    () async {
...
Future<Uint8List> _encryptedFrame(
