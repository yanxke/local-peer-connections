...
void main() {
  ...
  test('UT-262 late old-backend write failure cannot undo RESUME', () async {
    final failedBackend = _Backend()..leaveWritesPending = true;
    final resumedBackend = _Backend();
    final peer = PeerConnectionCore(
      backend: failedBackend,
      sessionRootKey: List.filled(32, 1),
    ...
    final pendingSubmission = peer.submitEncrypted(FrameType.data, [1]);
    unawaited(pendingSubmission);
    await Future<void>.delayed(Duration.zero);
    expect(failedBackend.pendingWrites, hasLength(1));
    peer.beginReconnect();
    peer.completeResume(
      newGeneration: 2,
      resumedSessionRootKey: List.filled(32, 5),
    ...
    // The old platform write can complete after the logical core has already
    // rebound. It must settle its own handle without taking the new backend
    // back through RECONNECTING.
    failedBackend.pendingWrites.single.fail();
    await Future<void>.delayed(Duration.zero);
    expect(peer.state, PeerConnectionState.ready);
  });

  test(
    'UT-044 checkpoint chunks share one MessageId and use fresh sequences',
    () async {
      final backend = _Backend();
      final peer = PeerConnectionCore(
        backend: backend,
  ...
  test(
    'UT-047 checkpoint retry resends every chunk with same MessageId',
    () async {
      final backend = _Backend();
      final peer = PeerConnectionCore(
        backend: backend,
      ...
      expect(
        await peer.retryAckRequiredCheckpoint(id, nowMs: 3000),
  ...
  test(
    'UT-055 checkpoint ACK timer starts only after final chunk submission',
    () async {
  ...
  test(
    'UT-061 resumed checkpoint retransmits chunk zero with same MessageId',
    () async {
      final backend = _Backend();
      final peer = PeerConnectionCore(
        backend: backend,
      ...
      expect(
        await peer.retransmitCheckpointAfterResume(id, nowMs: 1),
        AckTimeoutResult.retransmitWholeOperation,
      );
      final frames = backend.writes.map(LpcFrame.decode).toList();
      expect(frames.skip(2).map((frame) => frame.messageId), everyElement(id));
      expect(
        frames.skip(2).map((frame) => frame.transportGeneration),
        everyElement(2),
      );
  ...
  test('all retained checkpoints are replayed after RESUME', () async {
    ...
    final replayed = await peer.retransmitAckRequiredCheckpointsAfterResume(
      nowMs: 1,
    );
  ...
  test(
    'UT-078 fully submitted ordered DATA is not replayed after RESUME',
    () async {
      final backend = _Backend();
      final peer = PeerConnectionCore(
        backend: backend,
  ...
  test(
    'UT-080 multi-frame SendHandle waits for every DATA chunk submission',
    () async {
      final backend = _Backend()..pendingFromWriteNumber = 2;
      final peer = PeerConnectionCore(
        backend: backend,
  ...
  test('UT-167 realtime sequence allocation is independent per channel', () {
    ...
    expect(
      peer
          .allocateRealtimeDatagram(channelId: 1, senderTick: 0, bytes: [])
          .sequence,
      1,
    );
    expect(
      peer
          .allocateRealtimeDatagram(channelId: 2, senderTick: 0, bytes: [])
          .sequence,
      1,
    );
    expect(
  ...
  test(
    'RT-002 realtime DATA is not retransmitted after transport loss',
    () async {
      final backend = _Backend();
      final peer = PeerConnectionCore(
        backend: backend,
  ...
  test(
    'UT-070 resumed control operation retransmits without reapplying state',
    () async {
      final backend = _Backend();
      final peer = PeerConnectionCore(
        backend: backend,
      ...
      final operations = await peer.retransmitAckRequiredFramesAfterResume(
        nowMs: 1,
      );
  ...
  test(
    'UT-021 conflicting completed DATA MessageId closes the PeerConnection',
    () async {
      final backend = _Backend();
      final peer = PeerConnectionCore(
        backend: backend,
  ...
  test(
    'PeerConnection decrypts a remote frame once and suppresses replay',
    () async {
      final aBackend = _Backend();
      final a = PeerConnectionCore(
        backend: aBackend,
  ...
  test(
    ...
    () async {
      ...
      expect(
        (await a.receiveEncrypted(bBackend.writes.single))!.payload,
        ping.payload,
      );
  ...
  test(
    'UT-263 stalled keepalive write does not block receive liveness expiry',
    () async {
      var nowMs = 0;
      final backend = _Backend()..leaveWritesPending = true;
      final peer = PeerConnectionCore(
  ...
  test(
    'UT-163 live core retransmits ACK-required operations on deadline',
    () async {
      var nowMs = 0;
      final backend = _Backend();
      final peer = PeerConnectionCore(
  ...
  test(
    'PeerConnection sends generic ACK with mandated zero header fields',
    () async {
      final aBackend = _Backend();
      final a = PeerConnectionCore(
        backend: aBackend,
  ...
  test(
    'terminal backend write failure starts generation-wide reconnect',
    () async {
  ...
  test(
    'UT-039 backend close rejects further peer traffic with INVALID_STATE',
    () async {
      final backend = _Backend();
      final peer = PeerConnectionCore(
        backend: backend,
  ...
  test(
    'UT-038 DiscoverySession.stop does not close a PeerConnection',
    () async {
      final backend = _Backend();
      final peer = PeerConnectionCore(
        backend: backend,
  ...
  test(
    'UT-074 terminal transport loss fails every pending generation write',
    () async {
      final backend = _Backend()..leaveWritesPending = true;
      final peer = PeerConnectionCore(
        backend: backend,
  ...
  test(
    'UT-073 failed ACK-required write arms no timer or same-generation send',
    () async {
      final backend = _Backend()..leaveWritesPending = true;
      final peer = PeerConnectionCore(
        backend: backend,
      ...
      expect(peer.state, PeerConnectionState.reconnecting);
      expect(
        peer.ackRetention.onTimer(id, nowMs: 10000),
        AckTimeoutResult.ignored,
      );
      await expectLater(
        peer.submitEncrypted(FrameType.data, [1]),
        throwsA(isA<LpcException>()),
      );
    },
  );
  ...
  test(
    'UT-027 RESUME increments generation and resets wire sequence to one',
    () async {
      final backend = _Backend();
      final peer = PeerConnectionCore(
        backend: backend,
  ...
  test(
    'UT-019 ACK timeout retransmits same MessageId with a new sequence',
    () async {
      final backend = _Backend();
      final peer = PeerConnectionCore(
        backend: backend,
      ...
      expect((await receiver.receiveEncrypted(backend.writes.first))!.payload, [
        7,
      ]);
      expect((await receiver.receiveEncrypted(backend.writes.last))!.payload, [
        7,
      ]);
    },
  );
  ...
  test(
    'UT-028 unacknowledged ACK-required frame retransmits after RESUME',
    () async {
      final backend = _Backend();
      final peer = PeerConnectionCore(
        backend: backend,
      ...
      final retried = await peer.retransmitAckRequiredFramesAfterResume(
        nowMs: 10,
      );
  ...
  test(
    'UT-029 non-ACK frame is not automatically retransmitted after RESUME',
    () async {
      final backend = _Backend();
      final peer = PeerConnectionCore(
        backend: backend,
  ...
  test(
    'authenticated ACK removes only its retained logical operation',
    () async {
      final aBackend = _Backend();
      final bBackend = _Backend();
      final a = PeerConnectionCore(
