...
class CheckpointReplicationQueue {
  static const _maxSequence = 0x7fffffffffffffff;
  CheckpointReplicationOperation? _inFlight;
  Uint8List? _pending;
  int? _pendingPublicationId;
  int _nextSequence = 1;
  ...
  CheckpointReplicationOperation? publish(List<int> bytes,
    ...
    if (_inFlight != null) {
      ...
      _pendingPublicationId = publicationId;
      return null;
    }
    return _promote(bytes, publicationId: publicationId);
  }

  CheckpointReplicationOperation? completeInFlight() {
    _inFlight = null;
    final next = _pending;
    final nextPublicationId = _pendingPublicationId;
    _pending = null;
    _pendingPublicationId = null;
    return next == null
        ? null
        : _promote(next, publicationId: nextPublicationId);
  }
